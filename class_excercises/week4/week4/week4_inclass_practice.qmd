...
districts <- st_transform(districts, st_crs(pa_counties))
```
...
my_county <- pa_counties %>%
  filter(COUNTY_NAM == "MONTGOMERY")  # Change "CENTRE" to your county
...
ggplot() +
  ...
  labs(
    title = paste("Neighbors of Montgomery", "County"),
...
Intersecting includes the selected county you are observing.
---
...
my_county_hospitals <- hospitals_with_county %>%
  filter(COUNTY_NAM == "MONTGOMERY")  # Change to match your county
...
hospital_service_areas <- my_county_hospitals_proj %>%
  st_buffer(dist = 8046.72)  # 5 miles in feet for PA State Plane
...
ggplot() +
  ...
  labs(
    title = paste("Hospital Service Areas in Montgomery County"),
    subtitle = "Red points = Hospitals, Blue areas = 5-mile service zones"
  ) +
  theme_void()
```
...
**Your Task:** What percentage of your county is within 5 miles of a hospital?
...
Hospitals tend to concentrate in pockets of the county. Areas that may be undeserved are in the northern areas. 
